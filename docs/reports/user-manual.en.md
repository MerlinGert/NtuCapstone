...
The **Run Full Analysis** button above the message box sends a preset request that starts a bridge-owned background full-analysis task. The chat agent returns quickly with a task ID, so you can continue chatting while the task runs. A compact status badge beside the analysis buttons shows whether the background analysis is starting, running, stopping, completed, stopped, failed, or interrupted; while a background analysis is active, both analysis buttons are disabled to avoid overlapping runs. The background task writes and validates `reasoning-graph.json` first, runs follow-up checks, may use L2 workers for independent visual, statistical, model-action, skeptical, or hypothesis-expansion subtasks, validates graph patches, and ends with a plain-language summary in the user's language. L2 workers are bounded subtasks and cannot spawn deeper agents. Advanced users or agents can inspect or stop the task from the session root with `uv run python run_full_analysis.py status` and `uv run python run_full_analysis.py stop`.

The **Update Analysis** button appears next to **Run Full Analysis** after `reasoning-graph.json` is available in LLM Analysis. It starts a bridge-owned background incremental-analysis task. That task compares the latest graph or patch anchor with the current live trace, analyzes only new interactions and annotations, writes a `reasoning-graph-patch-incremental-<fromRevision>-<toRevision>.json` file when new evidence exists, validates the graph plus patches, and reports both a technical audit and a plain-language summary in the user's language. Advanced users or agents can inspect or stop it with `uv run python run_incremental_analysis.py status` and `uv run python run_incremental_analysis.py stop`. The chat history shows these preset requests as compact labels, such as `Run full analysis` and `Update analysis`, instead of displaying the full hidden prompt text.

Specialized Codex Chat turns use a closed trace window. When a message is sent, ManiScope records the current trace anchor in `.maniscope-chat/sessions/{sessionId}/analysis-runs/{runId}.json` and passes that anchor to the agent. A full analysis sets `reasoning-graph.json.analysisAnchor` to that run-start anchor; an incremental analysis uses the run-start anchor as the patch `targetAnchor`. If you continue interacting with ManiScope while Codex is still working, those later actions are intentionally not folded into the active run. Use **Update Analysis** after the active run finishes to analyze the deferred trace changes.
...
For comprehensive trace analysis, sessions also include a managed skeptical-review skill. When available, the analysis task agent may spawn a focused worker to look for negative evidence, false positives, benign alternatives, or model-parameter failures that weaken major hypotheses. The analysis owner verifies those candidate negative findings before adding `contradicts`, `refines`, or Reasoning Gap entries to analysis artifacts.
