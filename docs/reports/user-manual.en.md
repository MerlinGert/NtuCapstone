...
The camera-shaped **Create Insight** button starts multi-select mode for annotation nodes. Select one or more annotation nodes, click Confirm, enter an insight note, and save it. High-level insights are added back into the annotation record and can reference the annotations they summarize.

Clicking an annotation node opens its details. Clicking a high-level insight node opens the insight text and its referenced annotations.

### LLM Analysis

The LLM Analysis tab displays trace-analysis artifacts generated by Codex. It asks the backend for the current analysis artifact manifest, then loads the available User Reasoning Forest and Reasoning Graph Patch JSON files from the session `artifacts/` folder. Each user hypothesis is rendered as a nested reasoning card. Nodes from the User Reasoning Forest use the default card styling, while agent-created patch nodes from the Reasoning Graph Patch are shown in pink and attached to the hypothesis or reasoning node they support, qualify, or synthesize. When a patch Insight synthesizes the same target as lower-level patch Findings, the Insight is shown as a container around those Findings. Clicking a card opens its details, including evidence summaries and patch rationales when available.

The tab refreshes when it is opened, when you click Refresh, when Codex announces a new relevant artifact, and through a lightweight periodic check while the tab is active. The backend does not keep a long-running file watcher; it scans session artifacts on request and reports the latest recognized files.
...
10. Use the Action Tree to review the investigation path and combine annotations into high-level insights.
...
The bottom investigation panel is part of the analysis state. User actions, annotations, and insights are session data, and they are what the Export workflow serializes.
