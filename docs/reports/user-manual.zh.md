...
如果归档中包含 LLM checklist evaluation，导入页会额外显示一个 **Evaluation Summary** 标签，并默认切到这个只读统计视图。这里不会简单复现原始表单状态，而是聚合展示已记录的 hypothesis / finding 评价数量、完成度、答案分布，以及 finding 关联到哪些 hypothesis，便于直接做用户研究结果汇总。
