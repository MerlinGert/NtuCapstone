...
export default {
  ...
  watch: {
    ...
    entityDetectionResults: {
      handler(newVal) {
        console.log(
          'TokenDistribution: entityDetectionResults changed',
    ...
    linkDetectionResults: {
      handler(newVal) {
        console.log(
          'TokenDistribution: linkDetectionResults changed',
          newVal ? newVal.length : 0,
        )
        this.drawChart()
      },
    ...
    manipulationDetectionResults: {
      handler(newVal) {
        console.log(
          'TokenDistribution: manipulationDetectionResults changed',
          newVal ? newVal.length : 0,
        )
  ...
  methods: {
    processManipulationResults() {
      if (
        ...
        this.manipulationDetectionResults.length === 0
      ) {
    ...
    highlightSuspiciousTraders(suspiciousTraders) {
      console.log(
        'TokenDistribution: highlightSuspiciousTraders',
        suspiciousTraders.length,
      )
    ...
    captureSnapshot() {
      ...
      return {
        ...
        links: (() => {
          const links = []
          if (
            this.linkDetectionResults &&
            typeof this.linkDetectionResults === 'object' &&
            !Array.isArray(this.linkDetectionResults)
          ) {
            ...
            extractLinks(
              this.linkDetectionResults.target_relations_for_links,
              'link',
            )
            extractLinks(
              this.linkDetectionResults.target_related_relations_for_links,
              'link',
            )
            extractLinks(
              this.linkDetectionResults.target_related_relations_for_entity,
              'entity',
            )
          }
          return links
    ...
    drawChart() {
      ...
      console.log(
        'TokenDistribution: drawChart called with',
        this.entityDetectionResults,
        ' and',
        this.linkDetectionResults,
      )
      ...
      if (balances?.related_users) {
        relatedEntries = Object.entries(balances.related_users)
          .filter(([_owner, balance]) => balance > 0)
          .map(([owner, balance]) => ({
            id: owner,
            name: owner,
            value: balance,
      ...
      const _allNodesBalance =
        usersBalance + relatedEntries.reduce((sum, d) => sum + d.value, 0)
      ...
      if (
        this.entityDetectionResults &&
        this.entityDetectionResults.length > 0
      ) {
        // Map user -> group
      ...
      if (othersBalance > 0) {
        const targetRingArea =
          totalBalance > 0
            ? targetTotalInkArea * (othersBalance / totalBalance)
            : 0
        const userGroupRadius = Math.sqrt(
          maxGroupRadius * maxGroupRadius - targetRingArea / Math.PI,
        )
      ...
      const color = d3
        ...
        .domain([0, d3.max(allEntries, (d) => d.value)])
      ...
      if (this.linkDetectionResults) {
        ...
        if (this.linkDetectionResults.target_relations_for_links) {
          ...
          // Fallback
          linksToDraw = this.linkDetectionResults.map((l) => ({
            ...l,
            type: 'link',
          }))
        }
      ...
      groups.each(function (d) {
        ...
        groupG
          ...
          .style('opacity', 0.6)
          .style('stroke', (child) =>
            child.suspicious ? '#ff0000' : '#5976ba',
          )
        ...
        if (d.internalLinks && d.internalLinks.length > 0) {
          console.log(
            `Group ${d.id} has ${d.internalLinks.length} internal links.`,
          )
          groupG
      ...
      this.simulation = d3
        ...
        .force(
          ...
          d3
            .forceManyBody()
            .strength((d) => (d.isGroup ? -50 : d.isRelated ? -5 : -15)),
        )
        .force(
          'collide',
          d3
        ...
        .force(
          'r',
          d3
            .forceRadial((d) => (d.isRelated ? relatedRingRadius : 0), 0, 0)
            .strength((d) => (d.isRelated ? 1 : 0.15)),
        )
        ...
        .force(
          ...
          d3
            ...
            .id((d) => d.id)
            .distance((d) =>
              d.source.isRelated || d.target.isRelated
                ? relatedRingRadius * 0.5
                : 20,
            )
            .strength((d) =>
              d.source.isRelated || d.target.isRelated ? 0.5 : 1,
            ),
        )
    ...
    async runManipulationDetection(
      threshold,
      timeWindow,
      checkEntityBased,
      isAutoRun = false,
      ...
      console.log(
        'TokenDistribution: runManipulationDetection called with',
        threshold,
        timeWindow,
        checkEntityBased,
        isAutoRun,
      ...
      let users = []
      if (this.snapshotData?.balances?.users) {
        users = Object.keys(this.snapshotData.balances.users).filter(
          (u) => u !== 'Others',
        )
      }
