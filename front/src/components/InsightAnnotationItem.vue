<template>
  <div class="insight-anno-item" style="margin-bottom: 8px; border-left: 2px solid #cbd5e1; padding-left: 8px; margin-left: 4px;">
    <template v-if="itemData">
      <div class="anno-time">
        <span v-if="isInsight" style="background: #3182ce; color: white; padding: 2px 4px; border-radius: 4px; font-size: 10px; margin-right: 4px;">Insight</span>
      ...
      <div class="anno-text" :style="isInsight ? 'font-weight: 600; color: #2b6cb0;' : ''">{{ text }}</div>
      ...
      <div v-if="isInsight && refAnnotations && refAnnotations.length > 0" class="nested-insights" style="margin-top: 8px;">
...
export default {
  ...
  computed: {
    ...
    isInsight() {
      if (this.annoRecord) return !!this.annoRecord.isInsight;
      if (this.actionRecord) return !!this.actionRecord.actionInfo.isInsight;
      return false;
    },
    text() {
