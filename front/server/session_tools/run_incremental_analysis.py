#!/usr/bin/env python3
"""Start, inspect, or stop a ManiScope incremental-analysis background task."""
...
def start(args: argparse.Namespace) -> int:
    ...
    payload = _request("POST", f"/api/sessions/{SESSION_ID}/analysis-tasks/incremental/start", body)
    _print(payload)
    return 0
...
def status(args: argparse.Namespace) -> int:
    ...
    if latest is None:
        print("No incremental-analysis task found.", file=sys.stderr)
        return 1
    _print(latest)
    return 0
...
def stop(args: argparse.Namespace) -> int:
    ...
    if not task_id:
        ...
        if latest is None:
            print("No running incremental-analysis task found.", file=sys.stderr)
            return 1
