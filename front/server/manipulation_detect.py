...
async def detect_manipulation(request: ManipulationRequest):
    print(
        f"Starting manipulation detection with params: threshold={request.threshold}, window={request.time_window}min, check_entity={request.check_entity_based}"
    )
    ...
    individual_stats = (
        ...
        .agg({"bought_act": "sum", "sold_act": "sum", "token_bought_amount": "count"})
        ...
        for i, entity_members in enumerate(request.entities):
            ...
            if not matches.empty:
                ...
                for idx, best_match in sorted_matches.iterrows():
                    ...
                    print(
                        f"[DETECTED] Entity Group #{i + 1} Members {entity_members}: {reason_str}"
                    )
