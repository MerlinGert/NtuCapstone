...
class SnapshotRequest(BaseModel):
    ...
    related_user_threshold: float = (
        0.2  # Threshold factor for related users (relative to min processed balance)
    )
    coin: str = "ACT"
...
async def process_snapshot(request: SnapshotRequest):
    ...
    if not df.empty and processed_user_set:
        ...
        potential_related = set(related_df["from_owner"].unique()) | set(
            related_df["to_owner"].unique()
        )
        ...
        final_related_users = (
            potential_related.intersection(all_users_with_balance) - processed_user_set
        )
        ...
        print(
            f"Filtering related users: min_processed_balance={min_processed_balance}, factor={request.related_user_threshold}, threshold={balance_threshold}"
        )
        ...
        print(
            f"Identified {len(related_users)} related users (filtered {filtered_count} due to low balance)."
        )
    ...
    response = {
        ...
        "balances": {"users": processed_users, "related_users": related_users},
    }
