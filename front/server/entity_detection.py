...
from behavior_detection import (
    BalanceSequenceParams,
    EarningSequenceParams,
    TradingSequenceParams,
    process_rule3,
    process_rule4,
    process_rule5,
)
...
ACCOUNT_LABELS_PATH = os.path.join(
    BASE_DIR, "public", "processed", "transfers", "account_labels.json"
)
...
class DetectionRule(BaseModel):
    ...
    rule_type: str = Field(
        ..., description="Type of rule (e.g., 'threshold', 'pattern', 'frequency')"
    )
...
class DetectionRequest(BaseModel):
    ...
    target_users: Optional[List[str]] = Field(
        None, description="List of user IDs to check. If None, check all."
    )
    time_range: Optional[Dict[str, str]] = Field(
        None, description="Start and end time for analysis"
    )
...
def build_transfer_network_graph(
    target_users: Optional[List[str]],
    time_range: Optional[Dict[str, str]],
    threshold: int,
    check_funding_source: bool = False,
    volume_threshold: float = 0,
    check_same_sender: bool = False,
    check_same_recipient: bool = False,
    enable_tx_count: bool = True,
    enable_tx_volume: bool = True,
) -> nx.Graph:
    """
    ...
    if not os.path.exists(TRANSFER_STATS_PATH):
        print(
            f"Warning: Transfer stats file not found at {TRANSFER_STATS_PATH}. Please run precompute_transfer_stats.py first."
        )
        return nx.Graph()
    ...
    try:
        ...
        if target_users:
            ...
            df_transfer = df_transfer[
                df_transfer["from_owner"].isin(target_set)
                & df_transfer["to_owner"].isin(target_set)
            ]
        ...
        for _, row in df_transfer.iterrows():
            ...
            is_high_volume = (
                enable_tx_volume and volume_threshold > 0 and volume >= volume_threshold
            )
                    ...
                    for i in range(1, len(unique_recipients)):
                        ...
                        if G.has_edge(hub, peer):
                            ...
                            if isinstance(reasons, list):
                                ...
                                already_exists = any(
                                    r.startswith(f"Same sender ({sender})") for r in reasons
                                )
                ...
                if len(unique_senders) > 1:
                    ...
                    for i in range(1, len(unique_senders)):
                        ...
                        if G.has_edge(hub, peer):
                            ...
                            if isinstance(reasons, list):
                                already_exists = any(
                                    r.startswith(f"Same recipient ({recipient})") for r in reasons
                                )
                                if not already_exists:
                                    reasons.append(reason_str)
                                    G[hub][peer]["reasons"] = reasons
...
def process_transfer_network_rule(
    target_users: Optional[List[str]],
    time_range: Optional[Dict[str, str]],
    threshold: int,
    check_funding_source: bool = False,
    ...
    """
    try:
        G = build_transfer_network_graph(
            target_users,
            time_range,
            threshold,
        ...
        for i, comp in enumerate(components):
            if len(comp) > 1:
                ...
                # Format reasons
                final_reasons = (
                    sorted(list(reasons_set)) if reasons_set else ["Connected via network analysis"]
                )
...
def build_entity_graph(request: DetectionRequest) -> nx.Graph:
    ...
    # Separate rules
    transfer_network_rules = [
        r for r in request.rules if r.rule_type == "transfer-network" and r.enabled
    ]
    behavior_similarity_rules = [
        r for r in request.rules if r.rule_type == "behavior-similarity" and r.enabled
    ]
    ...
    for rule in transfer_network_rules:
        ...
        for u, v, d in G_rule.edges(data=True):
            if G_master.has_edge(u, v):
                # Merge attributes
                G_master[u][v]["transfer_count"] = G_master[u][v].get("transfer_count", 0) + d.get(
                    "transfer_count", 0
                )
                # Merge reasons
...
def process_transfer_network_links(
    target_users: Optional[List[str]], time_range: Optional[Dict[str, str]], threshold: int
) -> List[Dict[str, Any]]:
    if not os.path.exists(TRANSFER_STATS_PATH):
        return []
    ...
    try:
        ...
        for u, v, d in G.edges(data=True):
            if d["weight"] >= threshold:
                links.append({"source": u, "target": v, "weight": d["weight"]})
