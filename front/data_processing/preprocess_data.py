    ...
    try:
        ...
        if "block_time" in df.columns:
            ...
            df_final = df_final.rename(
                columns={"block_time": "timestamp", "amount_display": "amount"}
            )
...
def load_and_process_trades():
    ...
    try:
        ...
        df = pd.read_csv(
            TRADING_CSV_PATH, dtype=str
        )  # Read as string initially to avoid type issues
        ...
        # Rename columns
        df = df.rename(columns={"block_time": "timestamp", "trader_id": "trader"})
...
def generate_user_relations():
    ...
    if not os.path.exists(SORTED_TRANSFERS_PATH):
        logger.warning(
            f"Sorted transfers not found at {SORTED_TRANSFERS_PATH}. Please run transfer processing first."
        )
        return
    ...
    try:
        ...
        for row in df.itertuples(index=False):
            ...
            if valid_from and valid_to:
                if u_from not in recipients:
                    recipients[u_from] = []
                recipients[u_from].append({"address": u_to, "timestamp": ts, "amount": amt})

                # Record for To user (Recipient)
                if u_to not in senders:
                    senders[u_to] = []
                senders[u_to].append({"address": u_from, "timestamp": ts, "amount": amt})
        ...
        result = {"senders": senders, "recipients": recipients}
        ...
        logger.info(
            f"User relations generation complete. Found {len(senders)} users with senders and {len(recipients)} users with recipients."
        )
...
def generate_user_actions():
    ...
    if not os.path.exists(SORTED_TRADES_PATH):
        logger.warning(
            f"Sorted trades not found at {SORTED_TRADES_PATH}. Please run trade processing first."
        )
        return
    ...
    try:
        ...
        for row in df.itertuples(index=False):
            ...
            user_actions[user].append(
                {"timestamp": ts, "action_type": action, "amount": amt, "price": price}
            )
        ...
        logger.info(
            f"User actions generation complete. Found {len(user_actions)} users with actions."
        )
...
def generate_user_balances():
    ...
    try:
        ...
        df_from = df[df["from_owner"].isin(valid_users)][
            ["timestamp", "from_owner", "amount"]
        ].copy()
        ...
        def process_and_save(freq, output_path):
            ...
            for user, group in grouped:
                ...
                if len(balance_vals) > 0:
                    ...
                    for t, b in zip(selected_ts, selected_balance):
                        user_seq.append(
                            {"timestamp": t.strftime("%Y-%m-%d %H:%M:%S"), "balance": b}
                        )
            ...
            logger.info(
                f"Saved {output_path}. Size: {os.path.getsize(output_path) / 1024 / 1024:.2f} MB"
            )
        ...
        for user, group in grouped:
            ...
            for row in group.itertuples(index=False):
                ...
                if action == "buy":
                    ...
                    if earning_change != 0:
                        user_changes.append(
                            {"user": user, "timestamp": ts, "earning_change": earning_change}
                        )
            ...
            for user, group in grouped_changes:
                ...
                if len(vals) > 0:
                    ...
                    for t, v in zip(selected_ts, selected_vals):
                        user_seq.append(
                            {"timestamp": t.strftime("%Y-%m-%d %H:%M:%S"), "earning": v}
                        )
    ...
    try:
        ...
        for row in df_trades.itertuples(index=False):
            ...
            trade_dict[row.tx_id] = {
                ...
                "amount": row.amount,
                "price_usd": row.price,
                "total_usd": getattr(row, "amount_usd", row.amount * row.price)
                if getattr(row, "amount_usd", None) is not None
                else (
                    row.amount * row.price
                    if row.amount is not None and row.price is not None
                    else None
                ),
            }
        ...
        logger.info(
            f"User behavior sequences generation complete. Found {len(user_sequences)} valid users."
        )
