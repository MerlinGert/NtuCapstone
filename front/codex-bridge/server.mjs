...
- Before finalizing a full trace artifact set, verify that reasoning-graph.json validates, that user-reasoning-forest.json was generated from it, that every AnalyticQuestion has at least one incoming "answers" edge from a mid-level Finding or is marked as an unresolved Reasoning Gap, that the Finding hierarchy is not unnecessarily flat, and that trace annotations with user claims appear as user Finding nodes rather than being lost.
