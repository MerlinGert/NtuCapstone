...
Finding Space output answers an Analytic Question or supports, refines, or contradicts higher-level reasoning
```
...
After producing a plan, prefer patch-producing subagents for independent branches that can be validated separately, such as support evidence for different Hypotheses, answer evidence for separate AnalyticQuestions, adjacent-hypothesis branches, and skeptical review. The main agent should pre-allocate each patch-producing subagent's `branchId`, patch filename, `runId`, node ID prefix, and target graph IDs before spawning it.

For full trace analyses, include a Disconfirmation Review before finalizing the graph and follow-up plan. If a session-local skeptical-review skill is available at `.maniscope-chat/sessions/{sessionId}/skills/maniscope-disconfirmation/SKILL.md`, a main agent should spawn a bounded subagent to read that skill and actively search for negative evidence. By default, the subagent should report candidate negative Findings for the main agent to verify. If the main agent explicitly assigns a branch patch filename, runId, node ID prefix, and target graph IDs, the subagent may instead write one `reasoning-graph-patch-subagent-<branchId>.json` patch. The main agent must still verify each candidate before relying on it in the augmented graph. Verified skeptical Findings must be encoded with `refines` or `contradicts`; `supports` alone is invalid because it makes caveats look like positive support.
